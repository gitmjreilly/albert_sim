...
def dump_stacks() :
   ...
   PTOS = the_cpu.PTOS.read()
   PSP = the_cpu.PSP.read()
   print("   PTOS: %04X" % (PTOS))
   ...
   address = PSP - 1
   while (i <= size) :
       (value, type)= address_space.super_read(address)
       print "   %04X: %04X" % (address, value)
       address = address - 1
       i = i + 1

   print("Return Stack:")
   RTOS = the_cpu.RTOS.read()
   RSP = the_cpu.RSP.read()
   print("   RTOS: %04X" % (RTOS))
...
def run_simulator(single_step_mode):
    ...
    while (True) :
        ...
        if ( (tenth_second_tick == 1000000) ) :
            ...
            if printed_time_stamps_are_enabled:
...
while (True):
    ...
    if (selection == "C"):
        the_cpu.clear_all_break_points()
        continue
    ...
    if (selection == "T"):    
        printed_time_stamps_are_enabled = True
        continue
